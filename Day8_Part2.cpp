...
bool make_junction(int node1, int node2) {
...
int main() {
    ...
    for(int i=0;i<edges.size();i++) {
        bool junction_created = make_junction(get<1>(edges[i]), get<2>(edges[i]));
        if(junction_created) count++;
        if(count == points.size() - 1) {
            cout<<get<0>(points[get<1>(edges[i])]) * get<0>(points[get<2>(edges[i])])<<endl;
            break;
        }
    }
}